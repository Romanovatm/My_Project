...
В проекте реализованы функции для маскировки номеров банковских карт и счетов, форматирования дат, фильтрации и сортировки банковских операций, генераторы для работы с транзакциями и номерами банковских карт, декоратор для логирования выполнения функций, чтение операций из JSON-файла и конвертация суммы транзакции в рубли через внешний API.
...
- чтение списка транзакций из JSON-файла;
- обработка пустого, некорректного или отсутствующего JSON-файла;
...
- хранение тестового набора операций в `data/operations.json`;
...
.
├── data/
│   └── operations.json       # JSON-файл с тестовыми банковскими операциями
├── src/
│   ├── __init__.py
│   ├── decorators.py         # Декоратор для логирования выполнения функций
│   ├── external_api.py       # Конвертация суммы транзакции в рубли через внешний API
...
│   ├── utils.py              # Чтение банковских операций из JSON-файла
...
├── .gitignore
├── logging.txt               # Пример файла с логами
├── main.py
├── poetry.lock
...
Для работы функции конвертации валют нужен API-ключ.
...
В файле `.env` укажите свой API-ключ:

```env
API_KEY=your_api_key_here
```

Файл `.env` добавлен в `.gitignore`, поэтому не должен попадать в репозиторий.
...
Результат для файла `data/operations.json`:
...
print(transactions_info("wrong_path/operations.json"))
```
...
Пример ошибки при записи в файл:
...
В файл `logging.txt` будет добавлена строка:
...
- чтение данных из JSON-файла через `transactions_info`;
- конвертацию суммы транзакции в рубли через `sum_transactions_rub`.
